...
export class CheckoutsService {
    ...
    constructor(
       ...
       findByIdCheckoutestado = ( id ) : Observable<CheckoutsResponseInterface> => {
           return this._http.get(`${this.endPoint}/checkoutestado/${id}`, this.options)
               .map((response: Response) => response.json())
               .catch(this.handleError);
       }
       findByIdEmpleado = ( id ) : Observable<CheckoutsResponseInterface> => {
       ...
       update = ( checkout: CheckoutsInterface ) : Observable<CheckoutsResponseInterface> => {
           return this._http.patch(this.endPoint, checkout, this.options)
               .map((response: Response) => response.json())
               .catch(this.handleError);
       }
       ...
       insert = ( checkout: CheckoutsInterface ) : Observable<CheckoutsResponseInterface> => {
           return this._http.post(this.endPoint, checkout, this.options)
               .map((response: Response) => response.json())
               .catch(this.handleError);
       }
